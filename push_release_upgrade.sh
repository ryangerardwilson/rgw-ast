...
build_release_asset() {
  ...
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build \
    -ldflags "-s -w -X ${MODULE_PATH}/internal/version.Version=${version} -X ${MODULE_PATH}/internal/version.Commit=${commit} -X ${MODULE_PATH}/internal/version.BuildTime=${build_time}" \
    -o "$binary_path" ./cmd/rgw-ast
